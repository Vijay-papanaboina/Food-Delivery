...
# Start deployment
Print-Header "Food Delivery Platform - Production Deployment"
Write-Host "Prerequisites:"
Write-Host "- GKE cluster must be created and kubectl configured"
Write-Host "- Static IP must be reserved and LoadBalancer configured"
Write-Host "- DNS records must point to the LoadBalancer IP"
Write-Host ""

$null = Read-Host "Press Enter to continue or Ctrl+C to cancel"

# Step 1: Install NGINX Ingress Controller
Print-Step "Installing NGINX Ingress Controller"
...
Print-Step "Installing cert-manager"
...
Print-Step "Creating ClusterIssuer"
...
Invoke-Command "kubectl wait --for=condition=ready pod -l app=mongo -n $NAMESPACE --timeout=40s" "MongoDB ready"
Invoke-Command "kubectl wait --for=condition=ready pod -l app=kafka -n $NAMESPACE --timeout=40s" "Kafka ready"
...
Invoke-Command "kubectl wait --for=condition=ready pod -l app=user-service -n $NAMESPACE --timeout=40s" "User service ready"
Invoke-Command "kubectl wait --for=condition=ready pod -l app=order-service -n $NAMESPACE --timeout=40s" "Order service ready"
Invoke-Command "kubectl wait --for=condition=ready pod -l app=payment-service -n $NAMESPACE --timeout=40s" "Payment service ready"
Invoke-Command "kubectl wait --for=condition=ready pod -l app=delivery-service -n $NAMESPACE --timeout=40s" "Delivery service ready"
Invoke-Command "kubectl wait --for=condition=ready pod -l app=restaurant-service -n $NAMESPACE --timeout=40s" "Restaurant service ready"
Invoke-Command "kubectl wait --for=condition=ready pod -l app=notification-service -n $NAMESPACE --timeout=40s" "Notification service ready"
...
Invoke-Command "kubectl wait --for=condition=ready pod -l app=customers-frontend -n $NAMESPACE --timeout=40s" "Customer frontend ready"
Invoke-Command "kubectl wait --for=condition=ready pod -l app=drivers-frontend -n $NAMESPACE --timeout=40s" "Drivers frontend ready"
Invoke-Command "kubectl wait --for=condition=ready pod -l app=restaurants-frontend -n $NAMESPACE --timeout=40s" "Restaurant frontend ready"
Write-Host ""

# Step 5: Deploy Ingress
Print-Step "Deploying Ingress"
Invoke-Command "kubectl apply -f ingress/ingress-with-tls.yaml" "Deployed ingress"
Write-Host ""
